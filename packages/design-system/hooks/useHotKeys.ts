...
import { isMac } from '#lib/browser.ts';
import { KeyboardKey, type KeyboardKeyType } from '#lib/hotkeys.ts';
import {isEqual} from '#lib/isEqual.ts';
import { isTruthy } from '#lib/isTruthy.ts';

// Taken from https://github.com/ProtonMail/WebClients/blob/f987a1054084ea7ec47ffbee635b0f17c779deb2/packages/components/hooks/useHotkeys.ts
/*
...
export const useHotkeys = (
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    ...
    for (let i = 0; i < normalizedHotkeyTupleArray.length; i++) {
      ...
      if (isEqual(hotkeySequence, tailSequence)) {
